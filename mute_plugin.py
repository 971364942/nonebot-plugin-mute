...
async def check_password(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    # 检查是否在冷却时间内且不是管理员
    if now_time - last_time < 300 and not IsAdmin(user_id):
...
# 检查是否是管理员
async def IsAdmin(qqNum):
    member_info =  await M_Bot.get_group_member_info(group_id=M_Event.group_id, user_id=qqNum)
...
def add_command(group_id, new_command):
    ...
    # 获取群号对应的指令列表
    group_commands = commands_config.get(str(group_id), [])
    
    # 添加新的口他指令
    group_commands.append(new_command)

    # 如果group_id不在group_commands_config中，则创建一个空列表
    if group_id not in group_commands_config:
        group_commands_config[group_id] = []

    # 将新指令追加到列表中
    group_commands_config[group_id].append(new_command)

    # 更新配置文件
    commands_config[str(group_id)] = group_commands
    with open(CommandsFile, 'w', encoding='utf-8') as file:
        json.dump(commands_config, file, ensure_ascii=False, indent=4)    
